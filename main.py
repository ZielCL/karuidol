...
if not os.path.isfile('cartas.json'):
    cartas_ejemplo = [
        {"nombre": "Tzuyu", "version": "V1", "rareza": "Común", "imagen_url": "https://i.ibb.co/35dt3MZc/Tzuyu1.jpg"},
        {"nombre": "Tzuyu", "version": "V2", "rareza": "Rara", "imagen_url": "https://i.ibb.co/tMzCk0B/Tzuyu2.jpg"},
    ]
...
def imagen_de_carta(nombre, version):
    for carta in cartas:
        if carta['nombre'] == nombre and carta['version'] == version:
            return carta['imagen_url']
    return None

def rareza_de_carta(nombre, version):
    for carta in cartas:
        if carta['nombre'] == nombre and carta['version'] == version:
            return carta.get('rareza', '')
    return ""

# Comando /idolday: permite obtener una carta aleatoria una vez al día o por bonos
def comando_idolday(update, context):
    ...
    # Verificar si tiene bonos pendientes
    bono = col_bonos.find_one({"user_id": usuario_id})
    tiene_bono = bono and bono.get("bonos", 0) > 0
    ...
    if tiene_bono:
        puede_tirar = True
        col_bonos.update_one({"user_id": usuario_id}, {"$inc": {"bonos": -1}})
    elif usuario and 'last_idolday' in usuario:
        ...
        if diferencia.total_seconds() >= 86400:
            ...
            context.bot.send_message(
                chat_id=chat_id,
                text=f"Ya usaste /idolday hoy. Intenta de nuevo en {horas}h {minutos}m."
            )
            return
    else:
        puede_tirar = True
    ...
    imagen_url = carta.get('imagen_url')
    ...
    # Guardar reclamo pendiente (por grupo + usuario)
    clave = f"{grupo_id}_{usuario_id}"
    ...
    texto = f"<b>Carta obtenida:</b> <b>#{nuevo_id} {version}</b> <b>{nombre}</b>\n"
    if rareza:
        texto += f"<i>Rareza: {rareza}</i>"
    teclado = InlineKeyboardMarkup([
        [InlineKeyboardButton("🎴 Reclamar", callback_data=f"reclamar_{grupo_id}_{usuario_id}")]
    ])
    if imagen_url:
        try:
...
def manejador_reclamar(update, context):
    ...
    data = query.data  # formato "reclamar_{grupo_id}_{usuario_id}"
    partes = data.split("_")
    if len(partes) != 3:
        query.answer()
        return
    grupo_id = str(partes[1])
    id_usuario = int(partes[2])
    clave = f"{grupo_id}_{id_usuario}"
    ...
    version = carta['version']
    cid = carta['id']
    # Guardar carta en BD (acumular cantidad)
...
# Comando /bonoidolday <usuario_id> <cantidad>
def comando_bonoidolday(update, context):
    ...
    try:
        ...
        if usuario_id not in admin_ids:
            update.message.reply_text("Solo los administradores pueden usar este comando.")
            return
    except:
        update.message.reply_text("Error obteniendo administradores.")
        return
    ...
    try:
        ...
        cantidad = int(args[1])
        if cantidad < 1:
            update.message.reply_text("La cantidad debe ser al menos 1.")
            return
    except:
        update.message.reply_text("Argumentos inválidos. Usa /bonoidolday <user_id> <cantidad>")
        return

    col_bonos.update_one({"user_id": target_id}, {"$inc": {"bonos": cantidad}}, upsert=True)
    update.message.reply_text(f"✅ Bono de {cantidad} tiradas de /idolday entregado a {target_id}.")
